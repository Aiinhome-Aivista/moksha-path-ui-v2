...
// ==========================================
// BULLETPROOF DEEP SEARCH FUNCTION
// Searches all keys AND nested objects automatically
// ==========================================
const searchAnyKey = (dataArray: any[], searchQuery: string) => {
    ...
    // Helper function to dig into nested data 
    const deepSearch = (obj: any): boolean => {
        if (!obj) return false;
        
        return Object.values(obj).some((value) => {
            if (value === null || value === undefined) return false;
            if (typeof value === 'object') return deepSearch(value); // Recursively search nested objects
            return String(value).toLowerCase().includes(query);
        });
    };

    return dataArray.filter((item) => deepSearch(item));
};
// ==========================================
...
export const ManageBlog: React.FC = () => {
  ...
  // 👇 Apply the deep search function here 👇
  ...
  const confirmDelete = async () => {
    ...
    const fetchSeoData = async (event: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-secondary-800 rounded-xl shadow-sm border border-secondary-200 dark:border-secondary-700 overflow-hidden">
        {/* Search & Refresh Bar */}
        <div className="p-4 border-b border-secondary-200 dark:border-secondary-700 bg-white dark:bg-secondary-800/50 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="relative w-full sm:max-w-md flex items-center">
            <Search className="absolute left-3 text-gray-400" size={18} />
            <input
              type="text"
              placeholder="Search anything..."
              className="pl-10 pr-4 py-2 border border-gray-200 dark:border-secondary-600 rounded-lg w-full text-primary dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-[#b0cb1f]/50 transition-shadow text-black dark:text-white bg-white dark:bg-secondary-900"
              value={searchQuery}
          ...
          {isLoading && (
              <div className="absolute inset-0 bg-white/50 dark:bg-secondary-800/50 backdrop-blur-[1px] z-10 flex items-center justify-center">
                  <div className="flex flex-col items-center gap-2">
                      ...
                      <span className="text-sm font-medium text-secondary-600 dark:text-secondary-400">Loading blogs...</span>
                  </div>
              </div>
          )}
              ...
              {currentBlogs.length > 0 ? (
                currentBlogs.map((blog, index) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-center font-medium text-primary dark:text-gray-200 max-w-xs truncate align-middle">
                      {blog.blog_title || blog.title}
                    </td>
                    <td className="px-6 py-4 text-center text-primary dark:text-gray-400 text-xs max-w-xs truncate align-middle">
                      {blog.blog_content
                        ?.replace(/<[^>]*>?/gm, "")
                        .substring(0, 100) ||
                        blog.excerpt ||
                      ...
                      <td colSpan={7}>
                        <div className="p-12 text-center text-secondary-500 dark:text-secondary-400 flex flex-col items-center justify-center min-h-[200px]">
                          ...
                          <p className="font-semibold text-lg text-gray-600 dark:text-secondary-300 mb-1">
                            {searchQuery ? 'No blogs found' : 'No blogs created yet'}
                          </p>
                          <p className="text-sm text-gray-400">
                            {searchQuery ? `Your search for "${searchQuery}" did not return any results.` : 'Click "Create New Blog" to get started.'}
                          </p>
                        </div>
                      </td>
                    </tr>
