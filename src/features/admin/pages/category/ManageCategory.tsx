...
import { Plus, Search, Edit, Trash2, FolderTree, Loader2, AlertTriangle } from 'lucide-react';
...
export const ManageCategories: React.FC = () => {
    ...
    const confirmDelete = async () => {
        ...
        setIsDeleting(true);
        try {
            const response = await ApiServices.deleteBlogCategory({ id: categoryToDelete });
            if (response.data.code === 200 || response.data.status === 'success') {
                ...
                <div className="p-4 border-b border-secondary-200 dark:border-secondary-700 bg-white dark:bg-secondary-800/50">
                    <div className="relative w-full max-w-md flex items-center">
                        ...
                        <input
                            ...
                            className="pl-10 pr-4 py-2 border border-gray-200 dark:border-secondary-600 rounded-md w-full bg-white dark:bg-secondary-900 text-sm focus:outline-none focus:ring-2 focus:ring-[#b0cb1f]/50 transition-shadow text-secondary-900 dark:text-white"
                            value={searchQuery}
                    ...
                    {isLoading && (
                        <div className="absolute inset-0 bg-white/50 dark:bg-secondary-800/50 backdrop-blur-[1px] z-10 flex items-center justify-center">
                            <div className="flex flex-col items-center gap-2">
                                ...
                                <span className="text-sm font-medium text-secondary-600 dark:text-secondary-400">Loading categories...</span>
                            </div>
                        </div>
                    )}
                                    ...
                                    <tr key={cat.id} className="divide-x divide-gray-100 dark:divide-secondary-700 hover:bg-gray-50 dark:hover:bg-secondary-700/50 transition-colors">
                                        ...
                                        <td className="px-6 py-4 text-center align-middle">
                                            ...
                                            <div className="flex items-center justify-center space-x-4">
                                                <button onClick={() => handleOpenEditModal(cat.id)} className="text-amber-500 hover:text-amber-600 transition-colors p-1" title="Edit">
                                                    <Edit size={18} />
                                                </button>
                                                <button onClick={() => handleDeleteClick(cat.id)} className="text-red-500 hover:text-red-600 transition-colors p-1" title="Delete">
                                                    <Trash2 size={18} />
                                                </button>
                                            </div>
                                        </td>
                    ...
                    <div className="relative bg-white dark:bg-secondary-900 rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200 border border-secondary-100 dark:border-secondary-800">
                        <div className="p-8 text-center">
                            ...
                            <p className="text-secondary-500 dark:text-secondary-400 mb-8 leading-relaxed">
                                Are you sure you want to remove this category? All articles linked to this category might be affected.
                            </p>
