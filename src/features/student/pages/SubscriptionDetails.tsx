    ...
    const fetchSubscriptions = async () => {
        try {
            ...
            if (response.data?.status === "success") {
                ...
                showToast(response.data?.message || "Failed to load subscriptions", "error");
            }
        } catch (error: any) {
        ...
        <div className="p-4 sm:p-6 lg:p-8">
            ...
            <div className="mb-8">
                <div className="flex items-center gap-3 mb-1">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900">My Subscription Details</h1>
                        <p className="text-sm text-gray-500">View all your active subscriptions</p>
                    </div>
                </div>
            </div>
            ...
            {isLoading ? (
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-16 flex flex-col items-center justify-center text-center">
                    ...
                    <p className="text-sm text-gray-400 max-w-sm">
                        You don't have any active subscriptions yet. Subscribe to a plan to get started.
                    </p>
                </div>
            ) : (
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                        ...
                        <table className="w-full text-left">
                            <thead>
                                <tr className="bg-gradient-to-r from-gray-50 to-gray-50/80 border-b border-gray-100">
                                    ...
                                    <th className="px-6 py-4 text-[11px] font-bold text-gray-500 uppercase tracking-wider">Board</th>
                                    <th className="px-6 py-4 text-[11px] font-bold text-gray-500 uppercase tracking-wider">Class</th>
                                    <th className="px-6 py-4 text-[11px] font-bold text-gray-500 uppercase tracking-wider">Institute</th>
                                    <th className="px-6 py-4 text-[11px] font-bold text-gray-500 uppercase tracking-wider">Subjects</th>
                                    <th className="px-6 py-4 text-[11px] font-bold text-gray-500 uppercase tracking-wider">Academic Year</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-50">
                                {paginatedData.map((sub, index) => (
                                    ...
                                    >
                                        ...
                                        <td className="px-6 py-4 text-sm text-gray-700 font-medium">{sub.board_name}</td>
                                        <td className="px-6 py-4 text-sm text-gray-700 font-medium">{sub.class_name}</td>
                                        <td className="px-6 py-4 text-sm text-gray-600 max-w-[200px] truncate" title={sub.institute_name}>
                                            {sub.institute_name || "—"}
                                        </td>
                                        <td className="px-6 py-4">
                                        ...
                                        <td className="px-6 py-4">
                                            <span className="text-sm text-gray-600 font-medium">{sub.year}</span>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
                    ...
                    <div className="md:hidden divide-y divide-gray-50">
                        {paginatedData.map((sub, index) => (
                            <div key={sub.subscription_id} className="p-4 hover:bg-lime-50/40 transition-colors">
                                ...
                                <div className="grid grid-cols-2 gap-y-2 gap-x-4 text-sm">
                                    <div>
                                        ...
                                        <p className="text-gray-700 font-medium">{sub.board_name}</p>
                                    </div>
                                    <div>
                                        <p className="text-[10px] text-gray-400 uppercase font-semibold">Class</p>
                                        <p className="text-gray-700 font-medium">{sub.class_name}</p>
                                    </div>
                                    <div className="col-span-2">
                                        <p className="text-[10px] text-gray-400 uppercase font-semibold">Institute</p>
                                        <p className="text-gray-600">{sub.institute_name || "—"}</p>
                                    </div>
                                    <div className="col-span-2">
                                        <p className="text-[10px] text-gray-400 uppercase font-semibold mb-1">Subjects</p>
                                    ...
                                    <div>
                                        <p className="text-[10px] text-gray-400 uppercase font-semibold">Academic Year</p>
                    ...
                    {totalPages > 1 && (
                        <div className="flex items-center justify-center gap-1.5 px-6 py-5 border-t border-gray-100 bg-gray-50/50">
                            ...
                            <button
                                ...
                                className={`w-9 h-9 rounded-full flex items-center justify-center transition-all duration-200 ${currentPage === 1
                            ...
                            {getPageNumbers().map((page, i) =>
                                typeof page === "string" ? (
                                    ...
                                    <button
                                        ...
                                        className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-semibold transition-all duration-200 ${currentPage === page
                            ...
                            <button
                                ...
                                className={`w-9 h-9 rounded-full flex items-center justify-center transition-all duration-200 ${currentPage === totalPages
                                    ? "bg-gray-100 text-gray-300 cursor-not-allowed"
                                    : "bg-white text-gray-600 border border-gray-200 hover:bg-[#b0cb1f] hover:text-white hover:border-[#b0cb1f] shadow-sm"
                                    }`}
                            >
                    ...
                    <div className="px-6 py-3 bg-gray-50/80 border-t border-gray-100 flex items-center justify-between">
                        <p className="text-xs text-gray-500">
                            Showing <span className="font-semibold text-gray-700">{(currentPage - 1) * ITEMS_PER_PAGE + 1}</span>
                            –<span className="font-semibold text-gray-700">{Math.min(currentPage * ITEMS_PER_PAGE, subscriptions.length)}</span>
                            {" "}of <span className="font-semibold text-gray-700">{subscriptions.length}</span> subscriptions
                        </p>
